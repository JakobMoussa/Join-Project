...
function loadUsers(target) {
  ...
  users.forEach((user) => {
    ...
    target.innerHTML += singleUserContainer(initials, user.name, user.color);
  });
}
...
function assignedUser(name, color) {
  ...
  container.innerHTML += userIcon(color, initials);
}
